...
static char unverified_filename[MAX_PATH];
...
static void set_status(HWND progress, const char *status)
{
	...
	char buf[0x1000];
	g_total = 0;
	snprintf(buf, sizeof(buf), "WireGuard: %s...", status);
...
static DWORD __stdcall download_thread(void *param)
{
	...
	char filename[MAX_FILENAME_LEN], download_path[MAX_FILENAME_LEN + sizeof(msi_path)];
	char tempdir[MAX_PATH], renamebuf[MAX_PATH * sizeof(wchar_t) + sizeof(FILE_RENAME_INFO)];
	char buf[512 * 1024], random_filename[64 + 4 + 1];
	wchar_t total_bytes_str[22];
	size_t total_bytes, current_bytes;
	FILE_RENAME_INFO *renameinfo = (FILE_RENAME_INFO *)renamebuf;
	const char *arch;
	blake2b_ctx hasher;
	...
	if (!ConvertStringSecurityDescriptorToSecurityDescriptorA("O:BAD:PAI(A;;FA;;;BA)", SDDL_REVISION_1, &security_attributes.lpSecurityDescriptor, NULL))
		goto out;
	if (!GetWindowsDirectoryA(tempdir, sizeof(tempdir)) || !PathAppendA(tempdir, "Temp"))
		goto out;
	if (!random_string(random_filename))
		goto out;
	if (!PathCombineA(unverified_filename, tempdir, random_filename))
		goto out;
	if (!random_string(random_filename))
		goto out;
	memcpy(random_filename + 64, ".msi", 4);
	if (!PathCombineA(msi_filename, tempdir, random_filename))
		goto out;
	...
	request = WinHttpOpenRequest(connection, L"GET", L(latest_version_path), NULL, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_REFRESH | WINHTTP_FLAG_SECURE);
	if (!request)
		goto out;
	...
	if (!extract_newest_file(filename, hash, buf, bytes_read, arch))
		goto out;

	set_status(progress, "creating temporary file");
	filehandle = CreateFileA(unverified_filename, GENERIC_WRITE | DELETE, 0, &security_attributes, CREATE_NEW, FILE_ATTRIBUTE_TEMPORARY, NULL);
	if (filehandle == INVALID_HANDLE_VALUE)
		goto out;

	set_status(progress, "downloading installer");
	if ((size_t)snprintf(download_path, sizeof(download_path), msi_path, filename) >= sizeof(download_path))
		goto out;
	...
	renameinfo->FileNameLength = MultiByteToWideChar(CP_UTF8, 0, msi_filename, -1, renameinfo->FileName, MAX_PATH) - 1;
	if (renameinfo->FileNameLength == (DWORD)-1)
		goto out;
	if (!SetFileInformationByHandle(filehandle, FileRenameInfo, renameinfo, sizeof(renamebuf)))
		goto out;
	CloseHandle(filehandle); //TODO: I wish this wasn't required.
	filehandle = INVALID_HANDLE_VALUE;
	ShowWindow(progress, SW_HIDE);
	ret = MsiInstallProductW(renameinfo->FileName, NULL);
...
out:
	...
	if (ret) {
		...
		if (MessageBoxA(progress, "Something went wrong when downloading the WireGuard installer. Would you like to open your web browser to the MSI download page?", "Download Error", MB_YESNO | MB_ICONWARNING) == IDYES) {
			assert((size_t)snprintf(buf, sizeof(buf), "https://" server msi_path, "") < sizeof(buf));
			ShellExecuteA(progress, NULL, buf, NULL, NULL, SW_SHOWNORMAL);
		}
	}
	exit(ret);
	return ret;
...
static void cleanup(void)
{
	if (filehandle != INVALID_HANDLE_VALUE)
		CloseHandle(filehandle);
	for (int i = 0; i < 200 && unverified_filename[0] && !DeleteFileA(unverified_filename) && GetLastError() != ERROR_FILE_NOT_FOUND; ++i)
		Sleep(200);
	for (int i = 0; i < 200 && msi_filename[0] && !DeleteFileA(msi_filename) && GetLastError() != ERROR_FILE_NOT_FOUND; ++i)
		Sleep(200);
}
	...
	case WM_APP: if (g_total) {
		...
		if (chars) {
			...
			if (start[-1] == '.' && start[-2] == '.' && start[-3] == '.')
				...
				start = paren - 1;
			*start = '\0';
			snprintf(start, sizeof(buf) - (start - buf), " (%.2f%%)", g_current * 100.0f / g_total);
			SetWindowTextA(progress, buf);
		}
