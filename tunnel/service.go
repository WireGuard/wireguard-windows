...
func (service *tunnelService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (svcSpecificEC bool, exitCode uint32) {
	...
	if conf.AdminBool("ExperimentalKernelDriver") {
		log.Println("Creating WireGuardNT adapter")
		...
		if err == nil {
			...
			if err != nil {
				err = fmt.Errorf("Error deleting already existing interface: %w", err)
				serviceError = services.ErrorCreateWireGuardNT
				return
			}
		}
		for i := 0; i < 5; i++ {
			if i > 0 {
				...
				log.Printf("Retrying WireGuardNT creation after failure because system just booted (T+%v): %v", windows.DurationSinceBoot(), err)
			}
		...
		if err != nil {
			err = fmt.Errorf("Error creating interface: %w", err)
			serviceError = services.ErrorCreateWireGuardNT
			return
		}
		defer adapter.Delete(true)
		luid = adapter.LUID()
		wireguardNTVersion, err := driver.RunningVersion()
		if err != nil {
			log.Printf("Warning: unable to determine WireGuardNT version: %v", err)
		} else {
			log.Printf("Using WireGuardNT/%d.%d", (wireguardNTVersion>>16)&0xffff, wireguardNTVersion&0xffff)
		}
